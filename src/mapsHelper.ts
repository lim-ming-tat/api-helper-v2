...
export class SaveMap {
    ...
    // do not raise error when propertyName is missing/not exist
    @Expose()
    ignoreWhenNotExist?: boolean;
...
export class MapsHelper {
    ...
    // load system plugins
    private static plugins: Array<PluginBase> = [new IfExists(), new IfNotExists(), new IfTrue(), new IfFalse(), new IfEmpty()];
    ...
    private static greaterThan<T>(a: T, b: T) {
        return a > b;
    }

    public static filterData(item: Record<string, unknown>, dataFilter: DataFilter | undefined) {
        if (dataFilter !== undefined) return MapsHelper.filterDataV2(item, dataFilter) ? item : null;
        else return true;
    }

    public static applySaveMaps(param: ResponseParam, sessionData: SessionDataBase, saveMaps: Array<SaveMap>) {
        // try {
                ...
                if (!(saveMap.skip !== undefined && saveMap.skip)) {
                    ...
                    if (saveMap.dataPath !== undefined) {
                        ...
                        if (sourceData === undefined) {
                            ...
                            sourceData = _.get(param.sessionData, saveMap.sessionName ? saveMap.sessionName : '') as Array<Record<string, unknown>>;
                        }
                        ...
                        dataRows
                            ...
                            .forEach((item) => {
                                ...
                                if (saveMap.properties !== undefined) {
                                    ...
                                    saveMap.properties.forEach((propertyMap) => {
                                        if (propertyMap.propertyName !== undefined) {
                                            ...
                                            const selectedData = dataProp.filter((xitem: Record<string, unknown>) => {
                                                // console.log(JSON.stringify(item, null, 4))
                                ...
                                index++;
                                (<Array<Record<string, unknown>>>sourceData).push(dataObject);
                            });
                    } else {
                        ...
                        if (saveMap.propertyName !== undefined) {
                            if (saveMap.dataType !== undefined && saveMap.dataType === 'array') {
                                ...
                                (<Array<Record<string, unknown>>>sourceData).push(_.cloneDeep(_.get(param, saveMap.propertyName)));
                                ...
                                _.set(sourceData as object, saveMap.objectPropertyName, _.get(param, saveMap.propertyName));
                            } else {
                                // console.log(JSON.stringify(saveMap, null, 4))
                            ...
                            if (saveMap.dataType === 'array') {
                                ...
                                (<Array<unknown>>sourceData).push(saveMap.dataValue);
                            } else if (saveMap.dataType === 'object') {
                                if (saveMap.objectPropertyName === undefined) {
                                    // throw new PropertyUndefinedError('objectPropertyName', saveMap, saveMap);
                                    throw new TypeError(this.formatErrorMessage(saveMap, 'required .objectPropertyName is missing', 'saveMap'));
                                }
                                _.set(sourceData as object, saveMap.objectPropertyName, saveMap.dataValue);
                            } else {
        ...
        parametersMaps.forEach((item) => {
            ...
            if (match) {
                ...
                if (plugins.length === 1) {
                    ...
                    if ((dataSource.sessionData as SessionDataBase).debug || item.debug) console.log(`>>>>>>>> ${parameter}\n${newValue}\n<<<<<<<<`);
                } else {
    ...
    private static formatErrorMessage<T>(data: T, message: string, property?: string) {
    ...
    public static updatePropertyV2(
        sourcePropertyName: string,
        apiParam: ApiParam,
        // sessionData: SessionDataBase | object
        sessionData: SessionDataBase | unknown
    ): string {
        const dataSource = {
            apiParam: apiParam,
            sessionData: sessionData,
    ...
    private static sortData(outputMap: NexthopMap, dataRows: Array<object>): Array<object> {
    ...
    public static applyNextHopMaps(param: ApiParam, sessionData: SessionDataBase) {
        const dataSource = {
            apiParam: param,
            sessionData: sessionData,
        };
        ...
        if (param.nextHopMaps !== undefined && Array.isArray(param.nextHopMaps)) {
            ...
            param.nextHopMaps.forEach((nextHopMap) => {
                ...
                dataRows
                    .filter((item) => {
                        return MapsHelper.filterData(item as Record<string, unknown>, nextHopMap.dataFilter);
                    })
                    .forEach((item) => {
                        // ++index;
                        // ++count;
